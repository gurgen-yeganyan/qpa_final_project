...
def represent_gc_ratio(string: str, step: int) -> None:
    """"
    Calculates the GC ratio for the given DNA sequence(string) for (step) number
    of windows, saves the resulting plot in a .png file.
    """
    ...
    genome_positions = [(window+1) * step for window in range(len(distribution))]
